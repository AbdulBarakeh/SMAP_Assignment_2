...
//SRC: https://stackoverflow.com/questions/31490657/how-to-use-onsaveinstancestate-method-with-arraylist
        ...
        AdapterListActivity.setOnItemClickListener( new AdapterForWordList.OnItemClickListener() {
            ...
            public void onItemClick(int position) {
                ...
                GoToDetails.putExtra("PositionOfWord",WordList.get(position).getPositionOfWord());
...
//Handle Received Data
    ...
    protected void onActivityResult(int requestCode, int resultCode, @NonNull Intent receivedIntent) {
        super.onActivityResult(requestCode , resultCode , receivedIntent);
            if (resultCode == 99) {
                WordList.get(receivedIntent.getIntExtra("position",13)).setImageOfWord(receivedIntent.getIntExtra("pic" , R.drawable.imagenotfound));
                WordList.get(receivedIntent.getIntExtra("position",13)).setNameOfWord(receivedIntent.getStringExtra("name"));
                WordList.get(receivedIntent.getIntExtra("position",13)).setPronounOfWord(receivedIntent.getStringExtra("pronoun"));
                WordList.get(receivedIntent.getIntExtra("position",13)).setDescripOfWord(receivedIntent.getStringExtra("descrip"));
                WordList.get(receivedIntent.getIntExtra("position",13)).setNotesOfWord(receivedIntent.getStringExtra("notes"));
                WordList.get(receivedIntent.getIntExtra("position",13)).setRatingOfWord(receivedIntent.getDoubleExtra("rating",0));
